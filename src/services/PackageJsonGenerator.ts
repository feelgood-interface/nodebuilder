...
export default class PackageJsonGenerator {
	...
	private async getPackageJson() {
		return await axios.get<PackageJson>(this.packageJsonUrl).then((response) => response.data);
	}
	...
	private checkIfNodeExists() {
		...
		this.packageJson.n8n.nodes.forEach((nodePath) => {
			...
			if (newNodeName === existingNodeName) {
				throw new Error('The node you are trying to create already exists in the n8n repo');
			}
		});
	}
	...
	private storePackageJson() {
		fs.writeFileSync(this.outputPackageJsonPath, JSON.stringify(this.packageJson, null, 2));
	}
	...
	private findNodeSpot() {
		...
		throw new Error('No node path insertion spot found in `package.json` retrieved from n8n repo');
	}
}
