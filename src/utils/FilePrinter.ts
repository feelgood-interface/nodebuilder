...
export default class FilePrinter {
	...
	private printTypeScript() {
		new Project()
			.createSourceFile(this.nodegenPath + '.ts', `export default ${this.getJson()}`, {
				overwrite: true,
			})
			.saveSync();
...
export function printTranslation(yamlMainParams: CustomSpecParams['mainParams']) {
	console.log(yamlMainParams);
	fs.writeFileSync('translation.json', JSON.stringify(yamlMainParams, null, 2), 'utf8');
}

export function printStagedParams(mainParams: MainParams) {
	fs.writeFileSync('stagedParams.json', JSON.stringify(mainParams, null, 2), 'utf8');
}
