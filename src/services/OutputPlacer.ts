...
export class OutputPlacer {
	...
	private prePlacementCheck() {
		...
		const genericFunctionsTs = this.rootFilesToPlace.find((f) => f === 'GenericFunctions.ts');

		if (!genericFunctionsTs) throw new Error('No `GenericFunctions.ts` file found in /output dir');
	}
	...
	private async placeRootTsFiles() {
		...
		const destDir = path.join(this.targetDir, this.nodeTs.replace('.node.ts', ''));
	...
	private async placeDescriptionsTsFiles() {
		...
		const destDir = path.join(this.targetDir, this.nodeTs.replace('.node.ts', ''), 'descriptions');
