...
export default class ApiCallBuilder {
	...
	requestBody(rbArray: OperationRequestBody[]) {
		rbArray.forEach((rbItem) => {
			if (rbItem.name === 'Standard') {
				...
				this.lines.push(
					...rbItemNames.map((rbc) => `\t${rbc}: this.getNodeParameter('${camelCase(rbc)}', i),`),
				);
	...
	getRequestBodyItemNames(requestBodyItem: OperationRequestBody) {
		const formUrlEncoded = requestBodyItem.content['application/x-www-form-urlencoded'];
		if (formUrlEncoded) {
	...
	endpoint(endpoint: string) {
		this.lines.push(
			this.hasPathParam ? this.pathParamEndpoint(endpoint) : this.ordinaryEndpoint(endpoint),
		);
	}
